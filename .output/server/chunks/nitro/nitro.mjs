...
const _7rDO32 = eventHandler((event) => {
...
var P$1 = "Invariant failed";
function U$1(e, n) {
  if (!e) throw new Error(P$1);
}
function W(e = {}) {
  let n, t = false;
  const r = (o) => {
...
async function oe$1(e, n) {
  ...
  const p = m[o];
  if (!p) throw console.log("serverFnManifest", m), new Error("Server function info not found for " + o);
  let d;
  if (d = await p.importer(), !d) throw console.log("serverFnManifest", m), new Error("Server function module not resolved for " + o);
  const g = d[p.functionName];
  if (!g) throw console.log("serverFnManifest", m), console.log("fnModule", d), new Error(`Server function module export not resolved for serverFn ID: ${o}`);
  const $ = ["multipart/form-data", "application/x-www-form-urlencoded"], _ = await (async () => {
    try {
      const i = await (async () => {
        if (e.headers.get("Content-Type") && $.some((N) => {
          var x;
          return (x = e.headers.get("Content-Type")) == null ? void 0 : x.includes(N);
        })) return U$1(t.toLowerCase() !== "get", "GET requests with FormData payloads are not supported"), await e.formData();
        if (t.toLowerCase() === "get") return f.payload ? u.parse(f.payload) : void 0;
        const H = await e.text();
        return u.parse(H);
      })(), a = await g(i);
      return a instanceof Response ? a : isPlainObject$1(a) && "result" in a && a.result instanceof Response ? a.result : isRedirect$1(a) || isNotFound(a) ? A(a) : new Response(a !== void 0 ? u.stringify(a) : void 0, { status: Y$1(L()), headers: { "Content-Type": "application/json" } });
    } catch (i) {
      return i instanceof Response ? i : isPlainObject$1(i) && "result" in i && i.result instanceof Response ? i.result : isRedirect$1(i) || isNotFound(i) ? A(i) : (console.error("Server Fn Error!"), console.error(i), console.info(), new Response(u.stringify(i), { status: 500, headers: { "Content-Type": "application/json" } }));
    }
  })();
  if (_.headers.get("Content-Type") === "application/json") {
    const a = await _.clone().text();
...
const P = { stringify: (e) => JSON.stringify(e, function(r, s) {
  ...
  return e.forEach((r, s) => {
    const n = t[s];
    n !== undefined ? Array.isArray(n) ? n.push(r) : t[s] = [n, r] : t[s] = r;
  }), t;
}, (e) => {
    ...
    return [...t].reverse().forEach((l) => {
      [...l].reverse().forEach((d) => {
        ...
        if (d) if (d.title) u || (u = { tag: "title", children: d.title });
          ...
          const f = (_a = d.name) != null ? _a : d.property;
          if (f) {
            if (c[f]) return;
            c[f] = true;
          }
          i.push({ tag: "meta", attrs: { ...d } });
        }
      });
    }), u && i.push(u), i.reverse(), i;
  }, [t]), s = useRouterState({ select: (i) => i.matches.map((c) => c.links).filter(Boolean).flat(1).map((c) => ({ tag: "link", attrs: { ...c } })), structuralSharing: true }), n = useRouterState({ select: (i) => {
    const c = [];
    return i.matches.map((u) => e.looseRoutesById[u.routeId]).forEach((u) => {
      var l, d, f, b;
      return (b = (f = (d = (l = e.ssr) == null ? undefined : l.manifest) == null ? undefined : d.routes[u.id]) == null ? undefined : f.preloads) == null ? undefined : b.filter(Boolean).forEach((R) => {
        c.push({ tag: "link", attrs: { rel: "modulepreload", href: R } });
      });
    }), c;
...
const Ge = () => {
  const e = useRouter(), t = useRouterState({ select: (n) => {
    ...
    return u ? (n.matches.map((l) => e.looseRoutesById[l.routeId]).forEach((l) => {
      var d, f;
      return (f = (d = u.routes[l.id]) == null ? undefined : d.assets) == null ? undefined : f.filter((b) => b.tag === "script").forEach((b) => {
        c.push({ tag: "script", attrs: b.attrs, children: b.children });
      });
    }), c) : (warning(false, "<Scripts /> found no manifest"), []);
  }, structuralSharing: true }), { scripts: r } = useRouterState({ select: (n) => ({ scripts: n.matches.map((i) => i.scripts).flat(1).filter(Boolean).map(({ children: i, ...c }) => ({ tag: "script", attrs: { ...c, suppressHydrationWarning: true }, children: i })) }) }), s = [...r, ...t];
  return jsx(Fragment, { children: s.map((n, i) => createElement(pe, { ...n, key: `tsr-scripts-${n.tag}-${i}` })) });
};
function Qe(e) {
...
`;
    ...
    return g = a.replace(Re, (S, w, te, W, xe, ve) => {
      if (w) {
        ...
        if (o.es6) {
          const je = (re - 55296) * 1024 + ne - 56320 + 65536;
          return "\\u{" + c(je, T) + "}";
        }
...
function st(e, t) {
  ...
  return { replaced: Y(e, (n, i) => {
    if (n instanceof ReadableStream) {
      const [c, u] = n.tee(), l = { type: "stream", path: i, id: r.length, matchIndex: t.match.index, stream: u };
      return r.push(l), c;
    } else if (n instanceof Promise) {
      const c = defer$1(n), u = { type: "promise", path: i, id: r.length, matchIndex: t.match.index, promise: c };
      r.push(u);
    }
    return n;
  }), extracted: r };
...
function at(e) {
  ...
  function c(l) {
    t.serverSsr.injectScript(async () => (await l.promise, `__TSR_SSR__.resolvePromise(${k({ matchId: r.id, id: l.id, promiseState: l.promise[TSR_DEFERRED_PROMISE] }, { isScriptContext: true, wrap: true, json: true })})`));
  }
  function u(l) {
    t.serverSsr.injectHtml(async () => {
      try {
        ...
        let f = null;
        for (; !(f = await d.read()).done; ) if (f.value) {
          const b = `__TSR_SSR__.injectChunk(${k({ matchId: r.id, id: l.id, chunk: f.value }, { isScriptContext: true, wrap: true, json: true })})`;
          t.serverSsr.injectScript(() => b);
        }
        t.serverSsr.injectScript(() => `__TSR_SSR__.closeStream(${k({ matchId: r.id, id: l.id }, { isScriptContext: true, wrap: true, json: true })})`);
      } catch (d) {
...
function Y(e, t, r = []) {
  ...
  if (isPlainObject$1(e)) {
    const n = {};
    for (const i in e) n[i] = Y(e[i], t, [...r, i]);
    return n;
  }
  const s = t(e, r);
  return s !== e ? s : e;
...
async function bt(e, t) {
  ...
  try {
    const i = e.getReader();
    let c;
    for (; !(c = await i.read()).done; ) (r = t.onData) == null || r.call(t, c);
    (s = t.onEnd) == null || s.call(t);
  } catch (i) {
    (n = t.onError) == null || n.call(t, i);
  }
}
function me(e, t) {
  const r = gt();
...
const wt = async ({ request: e, router: t, responseHeaders: r }) => {
  ...
  if (typeof J$1.renderToPipeableStream == "function") {
    ...
    try {
      const i = J$1.renderToPipeableStream(jsx(ie, { router: t }), { ...le(e.headers.get("User-Agent")) ? { onAllReady() {
        i.pipe(s);
      } } : { onShellReady() {
        i.pipe(s);
      } }, onError: (c, u) => {
...
storage.mount('data', unstorage_47drivers_47fs_45lite({"driver":"fsLite","base":"/Users/mitchaplin/workspace/local-llm-chatbot/.data/kv"}));
...
export { H3Event as H, P, U$1 as U, nodeServer as a, getHeaders as g, node$1 as n, oe as o, u };
